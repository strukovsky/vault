...
pub fn __admin_withdraw(ctx: Context<AdminWithdraw>, amount: u64) -> Result<()> {
    ...
    require!(signer.key() == ctx.accounts.vault.owner, anchor_lang::error::ErrorCode::RequireViolated);
    let mut vault_balance = vault.try_borrow_mut_lamports()?;
    let is_rent_except_after_tx = Rent::get().unwrap().is_exempt(
        **vault_balance - amount,
        vault.data_len());
    require!(is_rent_except_after_tx, anchor_lang::error::ErrorCode::RequireViolated);
